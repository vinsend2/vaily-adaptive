...
class Mailer
{
	...
	public static function init(
			$host = 'smtp-pulse.com',
			$port = 2525,
			$username = 'mail@cornergrill.ru',
			$password = 'ijcnnNanC6sHBrG',
			$from = 'mail@cornergrill.ru',
			$fromName = 'GGPA Dev',
			$addresses = [],
			$html = true
		)
	...
	public static function setUp()
	{
		...
		self::$mail = new PHPMailer__PHPMailer();

		self::$mail->isSMTP();
		self::$mail->SMTPDebug = self::$debug ? 2 : 0;
		self::$mail->Host = $host;
		self::$mail->Port = $port;
		self::$mail->SMTPAuth = $username ? true : false;
		self::$mail->Username = $username;
		self::$mail->Password = $password;

		self::$mail->From     = $from; // only this address work

		self::$mail->FromName = $fromName;

		foreach ($addresses as $key => $adr) {
			self::$mail->AddAddress($adr, is_numeric($key) ? '' : $key);
		}
...
class PHPMailer__Exception extends Exception
{
    /**
...
class PHPMailer__PHPMailer
{
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new PHPMailer__Exception($error_message);
            }
            return false;
        }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new PHPMailer__Exception($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new PHPMailer__Exception($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new PHPMailer__Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new PHPMailer__Exception($this->lang('instantiate'), self::STOP_CRITICAL);
        }
        return true;
    }
    /**
     ...
     *
     * @uses \PHPMailer__PHPMailer__SMTP
     *
     * @param string $header The message headers
     * @param string $body   The message body
     *
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new PHPMailer__Exception($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new PHPMailer__Exception($this->ErrorInfo, self::STOP_CRITICAL);
        }
        $callbacks = [];
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new PHPMailer__Exception($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            throw new PHPMailer__Exception(
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ('tls' === $secure or 'ssl' === $secure) {
                ...
                if (!$sslext) {
                    throw new PHPMailer__Exception($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[3];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new PHPMailer__Exception($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new PHPMailer__Exception($this->lang('authenticate'));
                        }
                    }
                    return true;
                } catch (Exception $exc) {
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new PHPMailer__Exception($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw new PHPMailer__Exception($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new PHPMailer__Exception($this->lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = self::ENCODING_BASE64, $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new PHPMailer__Exception($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = self::ENCODING_BASE64)
    {
        try {
            if (!file_exists($path)) {
                throw new PHPMailer__Exception($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
